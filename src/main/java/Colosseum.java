...
public class Colosseum {
    ...
    public static Pokemon buildPokemon() {
        ...
        while (tempHit > MAX_HIT_POINTS || tempHit < 1) {
        ...
        int tempAtt = myScan.nextInt();
        while (tempAtt > 49 || tempAtt < 1) {
            System.out.println("Sorry. The attack level must be between 1 and 49:");
            tempAtt = myScan.nextInt();
        }
        tempPokemon.attackLevel = tempAtt;
        int diff = 50 - tempPokemon.attackLevel;
        System.out.println("Enter your defense level (1-" + diff + "):");
        int tempDef = myScan.nextInt();
        while (tempDef > diff || tempDef < 1) {
            System.out.println("Sorry. The defense level must be between 1 and " + diff + ":");
            tempDef = myScan.nextInt();
        }
        tempPokemon.defenseLevel = tempDef;
        return tempPokemon;
    }
